...
class QQ:
    ...
    urlMain     = 'http://user.qzone.qq.com/%s'
    urlLike     = 'http://w.qzone.qq.com/cgi-bin/likes/internal_dolike_app?g_tk=%d'
    ...
    def do_like(self):
        ...
        self.requests.get(self.urlQzone, headers=headers)   # important

        headers.update({
            'Host': 'user.qzone.qq.com'
        })
        r = self.requests.get(self.urlMain % self.qq, headers=headers)
        
        from bs4 import BeautifulSoup
        
        soup = BeautifulSoup(r.text)
        li = soup.find_all('li', 'f-single f-s-s')
        # print(len(li))
        for li_element in li:
            ...
            if name == 'xxx':
                ...
                if like['data-clicklog'] == 'like':
                    param = {
                        'qzreferrer'      : self.urlMain % self.qq,
                        'opuin'           : self.qq,
                        'unikey'          : data_unikey,
                        'curkey'          : data_curkey,
                        'appid'           : self.appid,
                        'typeid'          : 2,
                        'abstime'         : 1457536864,
                        'fid'             : '4e91ea23603fe05623350b00',
                        'active'          : 0,
                        'fupdate'         : 1

                    }
                    headers['Host'] = 'w.qzone.qq.com'
                    r = self.requests.post(self.urlLike % self.gtk(), data=param, headers=headers)
                    # print(r.text)
                    if "succ" in r.text:
                        break
