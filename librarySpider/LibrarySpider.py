...
class LibrarySpider:
    ...
    def post(self):
        ...
        req = requests.post(self.firstUrl, data=postdata, cookies=args[2], headers=self.headers)
        self.response = req

    def post_NowBorrow(self):
        req = requests.get(self.NowBorrowUrl, cookies=self.response.cookies).text     # 不能乱传headers,最好不要有
        soup = Beautiful(req)
    ...
    def post_ReadBooking(self):
        req = requests.get(self.ReadBookingUrl, cookies=self.response.cookies).text
        soup = Beautiful(req)
    ...
    def post_OutDateInfo(self):
        req = requests.get(self.OutDateInfoUrl, cookies=self.response.cookies).text
        soup = Beautiful(req)
        if soup("p")[-1].text == "没有相关记录！".decode("utf-8"):
    ...
    def post_ReaderArrearage(self):
        req = requests.get(self.ReaderArrearageUrl, cookies=self.response.cookies).text
        soup = Beautiful(req)
        if soup("p")[-1].text == "没有相关记录！".decode("utf-8"):
